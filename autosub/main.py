...
def main():
    ...
    for format in args.format:
        output_filename = os.path.join(output_directory, f"{video_prefix}.{args.engine}.{format}")
        ...
        if format == "vtt":
            h = output_file_handle_dict[format]
            h.write("WEBVTT\n")
            #h.write("Kind: captions\n")
            h.write("\n") # end of header
            h.write(f"NOTE this file was generated by autosub. engine: {args.engine}\n\n")

    input_file_link = os.path.join(output_directory, os.path.basename(input_file))
    if not os.path.exists(input_file_link):
        _logger.info(f"Creating symlink to input file: " + input_file_link)
        os.symlink(os.path.abspath(input_file), input_file_link)

    if os.path.exists(audio_file_name):
        _logger.info("Using existing audio file: " + audio_file_name)
    else:
        _logger.info("Extracting audio file: " + audio_file_name)
    ...
    remove_silent_segments(audio_file_name, audio_directory)
    ...
    _logger.info(f'Play video: mpv "{input_file_link}"') # TODO escape for shell. shlex?
